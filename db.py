...
def save_report(user_id: str, username: Optional[str], period: str = "note",
                amount: float = 0.0, category: Optional[str] = None,
                note: Optional[str] = None,
                created_at: Optional[dt.datetime] = None) -> Report:
    ...
    with _get_session(user_id) as s:
        r = Report(
            ...
            period=(period or "note").lower(), amount=amount,
            category=category, note=note, created_at=created_at,
        )
        s.add(r); s.commit(); s.refresh(r)
        return r

def list_reports(period: Optional[str] = None, date_only: Optional[dt.date] = None,
                 user_id: Optional[str] = None) -> List[Report]:
    if not user_id: return []
    with _get_session(user_id) as s:
        q = s.query(Report).order_by(Report.created_at.desc())
        if period: q = q.filter(Report.period == period.lower())
        if date_only:
...
def list_between(start_dt: dt.datetime, end_dt: dt.datetime,
                 user_id: Optional[str] = None, period: Optional[str] = None) -> List[Report]:
    if not user_id: return []
    if start_dt.tzinfo is None: start_dt = start_dt.replace(tzinfo=_tz.utc)
    if end_dt.tzinfo is None: end_dt = end_dt.replace(tzinfo=_tz.utc)
    with _get_session(user_id) as s:
...
def count_between(start_dt: dt.datetime, end_dt: dt.datetime, user_id: str) -> int:
    if start_dt.tzinfo is None: start_dt = start_dt.replace(tzinfo=_tz.utc)
    if end_dt.tzinfo is None: end_dt = end_dt.replace(tzinfo=_tz.utc)
    with _get_session(user_id) as s:
        return s.query(Report).filter(
            Report.created_at >= start_dt, Report.created_at <= end_dt
        ).count()

def delete_between(start_dt: dt.datetime, end_dt: dt.datetime, user_id: str) -> int:
    if start_dt.tzinfo is None: start_dt = start_dt.replace(tzinfo=_tz.utc)
    if end_dt.tzinfo is None: end_dt = end_dt.replace(tzinfo=_tz.utc)
    with _get_session(user_id) as s:
        rows = s.query(Report).filter(
            Report.created_at >= start_dt, Report.created_at <= end_dt
        ).all()
        n = len(rows)
...
def update_note(entry_id: int, user_id: str, new_text: str) -> bool:
    with _get_session(user_id) as s:
        ...
        r.note = new_text; s.commit(); return True
